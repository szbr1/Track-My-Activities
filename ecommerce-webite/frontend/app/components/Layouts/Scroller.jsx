...
function Scroller() {
  ...
  const handleMouseDown = (e) => {
    ...
    setIsDragging = true;
    setCanScroll: controlRef.current.scrollLeft;
  ...
  const handleMouseDragg = (e) => {
    if(!isDragging){
    const x = e.pageX - controlRef.current.offseLeft;
    const walk = x - startX;
  ...
  const scroll = (direction) => {
    const scrollleft = direction === "left" ? -500 : 500;
    controlRef.current.scrollBy({ left: scrollleft, behaviour: "smooth" });
  };
    ...
    <div className=" relative">
      ...
      >
        {products.map((product) => {
          return (
            <div
              className="mt-8 h-full w-full mx-2 rounded-md md:w-1/2 lg:w-64 flex-shrink-0 overflow-hidden"
              key={product.id}
            >
              <img
                src={product.url}
