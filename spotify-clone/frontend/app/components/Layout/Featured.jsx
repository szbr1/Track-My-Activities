...
function Featured({song,n}) {
  ...
  return (
    ...
    <div key={n}  className='rounded-sm overflow-hidden relative  group hover:bg-zinc-800 text-white bg-zinc-900/60 p-2 flex gap-4 justify-start items-center'>
      ...
      <div className='bg-white rounded-md' onClick={()=>setSongDetails(song) }>
      { isPlaying && song?._id === currentSong?._id ?<PauseIcon  className='absolute cursor-pointer top-6  right-4 text-green-600'/>:      <PlayIcon  className='absolute cursor-pointer top-6 hidden  group-hover:block right-4 text-green-600'/>  }
